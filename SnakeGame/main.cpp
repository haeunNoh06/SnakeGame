...
int main(void) {
	...
	for (int i = 0; i < BODY_MAX; i++) {
		snake.body_[i].x_ = -50, snake.body_[i].y_ = -50;	// 뱀 위 그리드 좌표
	...
	snake.body_[0].x_ = 3, snake.body_[0].y_ = 3;// snake의 head부분
	snake.body_[0].sprite_.setFillColor(Color::Green);
	...
	while (window.isOpen()) {
		...
		for (int i = 0; i < snake.length_; i++) {
			window.draw(snake.body_[i].sprite_);	// draw를 늦게 할수록 더 위에 있다
		}
